...
library(dplyr)
```
Tema para visualizacion de datos
...
library(readr)
#Si el data set no esta en la raiz sera necesario poner la ruta
#en el parametro del nombre de achivo
data_encuesta_nacional_hogares <-  read_delim("EH2019_Pers_csv.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE, "iconv file.pcl -f UTF-8 -t ISO-8859-1 -c") 

head(data_encuesta_nacional_hogares,5)
...
#----------------------------------------------
#Transformando datos de la data data_covid9_seguros_depto
1.----
```{r}
...
ggplot(data_covid9_seguros_depto_1, aes(depto, total_casos))+
     geom_bar( aes(fill= Casos), position = "dodge", stat = "identity", show.legend = TRUE)+
      scale_fill_manual(values = c("blue", "green", "turquoise", "orange"))+
       labs(
        title = "Relacion Covid-19(recuperados, en tratamiento) y asegurados",
        x = "Departamento",
...
ggplot(data_covid9_seguros_depto_2, aes(depto, total_casos))+
     geom_bar( aes(fill= Casos), position = "dodge", stat = "identity", show.legend = TRUE)+
      scale_fill_manual(values = c("blue", "turquoise"))+
       labs(
        title = "Relacion recuperados Covid-19 y asegurados",
        x = "Departamento",
        y = "Porcentaje de Casos") +
        tema_plot
